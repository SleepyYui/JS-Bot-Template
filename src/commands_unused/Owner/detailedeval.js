...
module.exports = {
  ...
  run: async (client, message, args, plusArgs, cmdUser, text, prefix) => {
    try {
      ...
      if (args.join(` `).includes(`token`)) return Logger.info(`${message.author.tag} tried to eval a token!`);
      //get the evaled content
      evaled = await eval(args.join(` `));
      //make string out of the evaluation
      let string = inspect(evaled);
      //if the token is included return error
      if (string.includes(client.token)) return Logger.info(`${message.author.tag} tried to eval a token!`);
      //define queueembed
