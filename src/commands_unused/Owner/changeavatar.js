...
module.exports = {
  ...
  description: "Changes the Avatar of theBot.",
      ...
      if (message.attachments.size > 0) {
        ...
        if (message.attachments.every(attachIsImage)) {
          ...
          await fs.writeFile(`./image.jpg`, buffer, () =>{
            logger.info(`${message.author.tag} changed the Avatar of the Bot.`);
          });
        ...
        const response = await fetch(url);
        const buffer = await response.buffer();
        await fs.writeFile(`./image.jpg`, buffer, () => Logger.info(`${message.author.tag} changed the Avatar of the Bot.`));
        client.user.setAvatar(`./image.jpg`)
          .then(user => {
            try {
              fs.unlinkSync("./image.jpg")
