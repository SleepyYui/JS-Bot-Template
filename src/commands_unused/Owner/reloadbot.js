...
module.exports = {
  ...
  run: async (client, message, args, plusArgs, cmdUser, text, prefix) => {
    try {
      ...
      await fs.readdirSync("./src/events/").forEach(async (dir) => {
      ...
      await fs.readdirSync("./commands/").forEach(async (dir) => {
        ...
        for (let file of commands) {
          try {
            ...
            //log if successful
            Logger.info(`Reloaded Command: ${file}`)
          } catch {}
        }
      })
      //WAIT 1 SEC
      ...
      await client.handlers.forEach(handler => {
        try {
          ...
          //log if successful
          Logger.info(`Reloaded Module: ${handler}`)
        } catch (e) {
          Logger.error(`Failed to reload Module: ${handler}`, e)
        }
      });
      ...
      Logger.error(`Failed to reload Bot!`, e)
